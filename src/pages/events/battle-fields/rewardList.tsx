...
import {
  Button, Checkbox, Divider, FormControl, FormControlLabel, IconButton, Input,
...
function RewardList({ mutateMode }: TMutateProps) {
  ...
  return (
    <Fragment>
      <Typography variant="subtitle2">Reward List</Typography>
      {userGoogleProfile && isInternalUser(userGoogleProfile.email) &&
        <div style={{ borderRadius: 3, border: 'solid 1px #ccc', padding: 8, marginBottom: '2rem' }}>
          ...
          {/* {eventForm.additionalParams.rewardList && */}
          {/* {eventForm.additionalParams.rewardList.map((outerReward, rOuterIndex) => */}
            {/* <div className="indent" key={`playerCount-${rOuterIndex}`}> */}
            <div className="indent" key={`playerCount`}>
              {
                !eventForm.additionalParams.rewardList.length &&
                <Alert severity="info" style={{ marginBottom: '1rem' }}>No real rewards yet.</Alert>
              }
              {eventForm.additionalParams.rewardList.map((reward, rInnerIndex) =>
                    ...
                    <Button
                      ...
                      onClick={e => {
                        dispatch(setEventForm(produce(eventForm, draftForm => {
                          draftForm.additionalParams.rewardList.splice(rInnerIndex, 1);
                        })));
                      }}
                    >
                      <DeleteIcon />
                  ...
                  <div>
                    ...
                    <FormControl style={{ display: 'inline-block', marginBottom: 0 }}>
                      <TextField
                        ...
                        onChange={(e: React.ChangeEvent<{ value: unknown }>) => {
                          dispatch(setEventForm(produce(eventForm, draftForm => {
                            const { minRank } = draftForm.additionalParams.rewardList[rInnerIndex];
                            minRank.value = e.target.value as number;
                            minRank.error =
                              parseInt(e.target.value as string) > parseInt(reward.maxRank.value + '') ?
                    ...
                    <FormControl style={{ display: 'inline-block', marginBottom: 0 }}>
                      <TextField
                        ...
                        onChange={(e: React.ChangeEvent<{ value: unknown }>) => {
                          dispatch(setEventForm(produce(eventForm, draftForm => {
                            const { maxRank } = draftForm.additionalParams.rewardList[rInnerIndex];
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        ...
                        checked={Boolean(eventForm.additionalParams.rewardList[rInnerIndex].realReward)}
                        disabled={mutateMode === 'View'}
                        onChange={(e: React.ChangeEvent<{ checked: boolean }>) => {
                          if (eventForm.additionalParams.rewardList[rInnerIndex].realReward) {
                            dispatch(setEventForm(produce(eventForm, draftForm => {
                              delete draftForm.additionalParams.rewardList[rInnerIndex].realReward;
                            })));
                          } else {
                            dispatch(setEventForm(produce(eventForm, draftForm => {
                              draftForm.additionalParams.rewardList[rInnerIndex].realReward = {
                                winningAmount: {
                                  amount: { ...numberField },
                    ...
                    <div className="indent">
                      ...
                      <FormControl style={{ display: 'inline-block', marginBottom: 0 }}>
                        <TextField
                          ...
                          onChange={(e: React.ChangeEvent<{ value: unknown }>) => {
                            dispatch(setEventForm(produce(eventForm, draftForm => {
                              const { realReward } = draftForm.additionalParams.rewardList[rInnerIndex]
                              if (realReward) {
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        ...
                        checked={Boolean(eventForm.additionalParams.rewardList[rInnerIndex].virtualRewards)}
                        disabled={mutateMode === 'View'}
                        onChange={(e: React.ChangeEvent<{ checked: boolean }>) => {
                          if (eventForm.additionalParams.rewardList[rInnerIndex].virtualRewards) {
                            dispatch(setEventForm(produce(eventForm, draftForm => {
                              delete draftForm.additionalParams.rewardList[rInnerIndex].virtualRewards;
                            })));
                          } else {
                            dispatch(setEventForm(produce(eventForm, draftForm => {
                              draftForm.additionalParams.rewardList[rInnerIndex].virtualRewards = [{ ...defaultVirtualReward }];
                            })));
                          }
                        }}
                      ...
                      <div className="indent" key={`virtualRewards-amount-field-${vrIndex}`}>
                        ...
                        <FormControl style={{ display: 'inline-block', marginBottom: 0 }}>
                          <TextField
                            ...
                            onChange={(e: React.ChangeEvent<{ value: unknown }>) => {
                              dispatch(setEventForm(produce(eventForm, draftForm => {
                                const { virtualRewards } = draftForm.additionalParams.rewardList[rInnerIndex]
                                if (virtualRewards) {
          ...
          <Divider style={{ margin: '1rem 0' }} />
          {
           !isSelectedAppCrypto &&
            <Button
              ...
              onClick={e => {
                dispatch(setEventForm(produce(eventForm, draftForm => {
                  const pc = eventForm.additionalParams.rewardList;
                  draftForm.additionalParams.rewardList = [
                    ...draftForm.additionalParams.rewardList,
                    {
